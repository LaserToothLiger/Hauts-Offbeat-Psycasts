...
namespace HautsPsycasts
{
    ...
    public class HediffComp_LinkRevoker : HediffComp_PairedHediff
    {
        ...
        public override void CompPostTick(ref float severityAdjustment)
        {
            ...
            if (this.Pawn.IsHashIntervalTick(10) && this.hediffs != null)
            {
        ...
        {
            if (((this.Props.aiCanAutoRecall && this.Pawn.HostileTo(h.pawn)) || this.AIShouldRecallOtherQualification(h)) && this.hediffs != null && this.hediffs.Contains(h) && h.pawn.Position != null)
            {
    ...
    //level 3
    /*public class CompProperties_AbilityInflame : CompProperties_AbilityEffect
    {
        public StatDef durationMultiplier;
        public FloatRange baseDamage;
        public DamageDef damageType;
        public float baseArmorPen;
        public float dmgVsArchitesFactor;
        public float dmgVsEntitiesFactor;
        public int tickPeriodicity;
    }
    public class CompAbilityEffect_Inflame : CompAbilityEffect
    {
        public new CompProperties_AbilityInflame Props
        {
            get
            {
                return (CompProperties_AbilityInflame)this.props;
            }
        }
        public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
        {
            ...
            {
                int duration = (int)(60 * this.parent.def.GetStatValueAbstract(StatDefOf.Ability_Duration, this.parent.pawn) * target.Thing.GetStatValue(this.Props.durationMultiplier));
                WorldComponent_InflamedThings WCIT = (WorldComponent_InflamedThings)Find.World.GetComponent(typeof(WorldComponent_InflamedThings));
                for (int i = WCIT.victims.Count - 1; i >= 0; i--)
                {
                    if (WCIT.victims[i].victim == target.Thing)
                    {
                        InflamedThing it = WCIT.victims[i];
                        it.duration = duration;
                        it.baseDamage = this.Props.baseDamage;
                        it.damageType = this.Props.damageType;
                        it.baseArmorPen = this.Props.baseArmorPen;
                        it.dmgVsArchitesFactor = this.Props.dmgVsArchitesFactor;
                        it.dmgVsEntitiesFactor = this.Props.dmgVsEntitiesFactor;
                        it.tickPeriodicity = this.Props.tickPeriodicity;
                        //WCIT.victims.Remove(WCIT.victims[i]);
                        return;
                    }
                }
                WCIT.victims.Add(new InflamedThing(this.parent.pawn, target.Thing, this.Props.baseDamage, this.Props.damageType, this.Props.baseArmorPen, this.Props.dmgVsArchitesFactor, this.Props.dmgVsEntitiesFactor, this.Props.tickPeriodicity, duration));
            }
        }
    }
    public class InflameDamageBooster : DefModExtension
    {
        public InflameDamageBooster()
        {

        }
        public float addsDamageFactor;
        public float addsArmorPen;
    }
    public class WorldComponent_InflamedThings : WorldComponent
    {
        public WorldComponent_InflamedThings(World world) : base(world)
        {
            this.world = world;
        }
        public override void WorldComponentTick()
        {
            ...
            {
                if (this.victims[i].victim.DestroyedOrNull() || !this.victims[i].victim.Spawned || (this.victims[i].victim is Pawn p && p.DeadOrDowned))
                {
                    this.victims.Remove(this.victims[i]);
                    continue;
                }
                this.victims[i].duration--;
                if (this.victims[i].duration % this.victims[i].tickPeriodicity == 0)
                {
                    this.victims[i].DoDamage();
                }
                if (this.victims[i].duration <= 0)
                {
                    this.victims.Remove(this.victims[i]);
                }
            }
        ...
        {
            base.ExposeData();
            Scribe_Collections.Look<InflamedThing>(ref this.victims, "victims", LookMode.Deep, Array.Empty<object>());
        }
        public List<InflamedThing> victims = new List<InflamedThing>();
    }
    public class InflamedThing : IExposable
    {
        public InflamedThing()
        {

        }
        public InflamedThing(Pawn creator, Thing victim, FloatRange baseDamage, DamageDef damageType, float baseArmorPen, float dmgVsArchitesFactor, float dmgVsEntitiesFactor, int tickPeriodicity, int duration)
        {
            ...
            this.dmgVsArchitesFactor = dmgVsArchitesFactor;
            this.dmgVsEntitiesFactor = dmgVsEntitiesFactor;
            this.tickPeriodicity = tickPeriodicity;
        ...
        public void ExposeData()
        {
            ...
            Scribe_Values.Look<FloatRange>(ref this.baseDamage, "baseDamage", new FloatRange(0));
            Scribe_Defs.Look<DamageDef>(ref this.damageType, "damageType");
            Scribe_Values.Look<float>(ref this.baseArmorPen, "baseArmorPen", 0f);
            Scribe_Values.Look<float>(ref this.dmgVsArchitesFactor, "dmgVsArchitesFactor", 0f);
            Scribe_Values.Look<float>(ref this.dmgVsEntitiesFactor, "dmgVsEntitiesFactor", 0f);
            Scribe_Values.Look<int>(ref this.duration, "duration", 0);
            Scribe_Values.Look<int>(ref this.tickPeriodicity, "tickPeriodicity", 0);
        }
        public void DoDamage()
        {
            float damage = this.baseDamage.RandomInRange;
            float damageBoost = 1f;
            float armorPenBoost = 1f;
            if (this.victim is Pawn p)
            {
                if (p.DeadOrDowned)
                {
                    return;
                }
                HVPUtility.InflameDamageBoosts(p, ref damageBoost, ref armorPenBoost, this.dmgVsArchitesFactor, this.dmgVsEntitiesFactor);
            }
            damage *= damageBoost;
            this.victim.TakeDamage(new DamageInfo(this.damageType, damage, this.baseArmorPen * armorPenBoost,-1f,this.creator));
            if (this.victim.MapHeld != null && this.victim.PositionHeld != null)
            {
                if (!this.victim.PositionHeld.ToVector3().ShouldSpawnMotesAt(this.victim.MapHeld, true))
                {
                    return;
                }
                FleckCreationData dataStatic = FleckMaker.GetDataStatic(this.victim.PositionHeld.ToVector3(), this.victim.MapHeld, FleckDefOf.Smoke, Rand.Range(1.5f, 2.5f) * damage / this.baseDamage.Average);
                dataStatic.rotationRate = Rand.Range(-30f, 30f);
                dataStatic.velocityAngle = (float)Rand.Range(30, 40);
                dataStatic.velocitySpeed = Rand.Range(0.5f, 0.7f);
                this.victim.MapHeld.flecks.CreateFleck(dataStatic);
            }
        }
        public Pawn creator;
        public Thing victim;
    ...
    public class CompAuraStaggerer : CompAuraEmitter
    {
        ...
        public override void AffectPawn(Pawn pawn)
        {
            ...
            if (pawn.stances != null && !AnimalCollectionClass.floating_animals.Contains(pawn))
            {
    ...
    public class HediffComp_Hammerspace : HediffComp, IThingHolder
    {
        ...
        public override void CompPostPostRemoved()
        {
            ...
            {
                this.StorageDump(this.Pawn.IsPlayerControlled?this.Props.eachItemLostOnDeathChance: this.Props.lostChanceIfNonPlayer, this.Pawn.Dead);
            }
        }
        ...
        public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
        {
            ...
            for (int i = 0; i < ets; i++)
            {
                ...
                if (outcome == "resources")
                {
                    ...
                    activeDropPodInfo.innerContainer.TryAdd(pawnToDrop, true);
                } else {
                ...
                DropPodUtility.MakeDropPodAt(target.Cell, this.parent.pawn.Map, activeDropPodInfo, null);
            }
        }
        public override bool Valid(LocalTargetInfo target, bool throwMessages = false)
        {
    ...
    public class Meteoroid : Skyfaller
    {
        protected override void Impact()
        {
            if (HautsUtility.CanBeHitByAirToSurface(base.Position, base.Map, false))
            {
                ...
                GenExplosion.DoExplosion(base.Position, base.Map, this.def.skyfaller.explosionRadius, this.def.skyfaller.explosionDamage, this.caster ?? null, totalDamage, -1f, null, null, null, null, null, 0f, 1, null, false, null, 0f, 1, 0f, false, null, (!this.def.skyfaller.damageSpawnedThings) ? this.innerContainer.ToList<Thing>() : null, null, true, 1f, 0f, true, null, 1f, null, null);
            }
    ...
    public class HVPUtility
    {
        ...
        public static void ArmourPiercingBolt(IntVec3 strikeLoc, Map map, Thing caster, int damage, float armorPenetration)
        {
            ...
            {
                GenExplosion.DoExplosion(strikeLoc, map, 1.9f, DamageDefOf.Flame, caster, damage, armorPenetration, null, null, null, null, null, 0f, 1, null, false, null, 0f, 1, 0f, false, null, null, null, true, 1f, 0f, true, null, 1f, null, null);
        ...
        /*public static void InflameDamageBoosts(Pawn p, ref float damageBoost, ref float armorPenBoost, float dmgVsArchitesFactor, float dmgVsEntitiesFactor)
        {
            foreach (Hediff h in p.health.hediffSet.hediffs)
            {
                InflameDamageBooster idb = h.def.GetModExtension<InflameDamageBooster>();
                if (idb != null)
                {
                    damageBoost += idb.addsDamageFactor;
                    armorPenBoost += idb.addsArmorPen;
                }
            }
            if (ModsConfig.BiotechActive && p.genes != null)
            {
                foreach (Gene g in p.genes.GenesListForReading)
                {
                    ...
                    {
                        damageBoost += dmgVsArchitesFactor;
                        break;
                    }
                }
            }
            if (ModsConfig.AnomalyActive && (p.IsMutant || p.IsEntity))
            {
                damageBoost += dmgVsEntitiesFactor;
            }
        }*/
